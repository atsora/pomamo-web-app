...
class MachinesDetailsPage extends pulsePage.BasePage {
  ...
  buildContent() {
    try {
      const el = document.querySelector('x-datetimerange[period-context="operatordashboard"]');
      if (el) {
        const today = moment().startOf('day');
        const beginIso = today.clone().hour(5).toISOString();
        const endIso = today.clone().hour(13).toISOString();
        const rangeStr = beginIso + ';' + endIso;
        el.setAttribute('range', rangeStr);
        console.log('operatordashboard: set x-datetimerange range=', rangeStr);
      }
    } catch (e) {
      console.warn('operatordashboard: unable to set datetimerange attribute', e);
    }
    // show Bars
